...
import { acceptMatch, declineAssignment, getUniversityMatches, type UniversityMatch } from '@/lib/university-acceptance-service'
...
export function UniversityChallengesPage() {
  ...
  async function decline(row: Row) { if (!userProfile?.universityId || !userProfile.uid) return; setAccepting(row.match.challengeId); try { await declineAssignment({ challengeId: row.match.challengeId, universityId: userProfile.universityId, universityAdminUid: userProfile.uid }); await load() } catch (e) { setError(e && typeof e === 'object' && 'message' in e ? String(e.message) : 'Unable to decline assignment.') } finally { setAccepting(null) } }
  ...
  return <div><PageHeader title="Assigned Challenges" subtitle="Government assignments awaiting your institution's decision." />{rows.length === 0 ? <EmptyState title="No assigned challenges" description="There are no Government assignments for your university yet." /> : <div className="space-y-3">{rows.map((row) => <Card key={row.match.challengeId}><CardContent className="flex items-center justify-between gap-4 py-4"><button className="min-w-0 text-left" onClick={() => navigate(`/university/challenges/${row.match.challengeId}`)}><p className="font-medium">{row.challenge?.title ?? 'Challenge'}</p><p className="text-sm text-muted-foreground">{row.challenge?.domain ?? 'Details available to authorized members'}</p></button><div className="flex items-center gap-3">{row.match.accepted ? <Badge variant="success">Accepted</Badge> : <Badge>Awaiting decision</Badge>}{userProfile?.role === 'university_admin' && !row.match.accepted && <><Button size="sm" disabled={accepting === row.match.challengeId} onClick={() => void decline(row)} variant="outline">Decline Assignment</Button><Button size="sm" disabled={accepting === row.match.challengeId} onClick={() => void accept(row)}>{accepting === row.match.challengeId ? 'Accepting...' : 'Accept Assignment'}</Button></>}</div></CardContent></Card>)}</div>}</div>
}
